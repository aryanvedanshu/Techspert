...
const AdminContentManagement = () => {
  ...
  const fetchAllContent = async () => {
    ...
    try {
      ...
      setFormData({
        team: teamRes.data?.data?.data || teamRes.data?.data || [],
        features: featuresRes.data?.data?.data || featuresRes.data?.data || [],
        statistics: statisticsRes.data?.data?.data || statisticsRes.data?.data || [],
        faqs: faqsRes.data?.data?.data || faqsRes.data?.data || [],
        contactInfo: contactRes.data?.data?.data || contactRes.data?.data || {},
        pageContent: pageRes.data?.data?.data || pageRes.data?.data || {},
        siteSettings: settingsRes.data?.data?.data || settingsRes.data?.data || {}
      })
    } catch (error) {
          ...
          <Card key={index} className="p-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <FileUpload
                  label="Profile Image"
                  storagePath="team/images"
                  value={member.imageUrl || ''}
                  onChange={(url) => updateArrayItem('team', index, { ...member, imageUrl: url })}
                  onRemove={() => updateArrayItem('team', index, { ...member, imageUrl: '' })}
                  maxSizeMB={5}
                />
              </div>
