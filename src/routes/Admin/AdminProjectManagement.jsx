...
const AdminProjectManagement = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    logger.functionEntry('handleSubmit', { editingProject: projectId })
    const startTime = Date.now()
    try {
      if (editingProject) {
        logger.apiRequest('PUT', `/admin/projects/${projectId}`, formData)
        await api.put(`/admin/projects/${projectId}`, formData)
        logger.apiResponse('PUT', `/admin/projects/${projectId}`, 200, { message: 'Project updated' }, Date.now() - startTime)
  ...
  const handleEdit = async (project) => {
    ...
    logger.functionEntry('handleEdit', { projectId, projectTitle: project.title })
    const startTime = Date.now()
    try {
      ...
      if (!project.description || !project.technologies) {
        ...
        logger.apiRequest('GET', `/admin/projects/${projectId}`)
        const response = await api.get(`/admin/projects/${projectId}`)
        logger.apiResponse('GET', `/admin/projects/${projectId}`, response.status, { hasData: !!response.data.data })
        project = response.data.data
        logger.info('Full project data fetched', { projectId: project.id || project._id })
      }
      logger.debug('Setting up edit form', { projectId: project.id || project._id })
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                <FileUpload
                  label="Student Image"
                  storagePath="projects/students"
                  value={formData.studentImage}
                  onChange={(url) => setFormData(prev => ({ ...prev, studentImage: url }))}
                  onRemove={() => setFormData(prev => ({ ...prev, studentImage: '' }))}
                  maxSizeMB={5}
                />
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <p className="text-xs text-neutral-500 mt-1">
                  For video files, upload to YouTube/Vimeo first, then paste the URL here
                </p>
              </div>
              <div>
                <MultiFileUpload
