...
const AdminSettings = () => {
  ...
  const [settings, setSettings] = useState({
    ...
    theme: {
      ...
      fontSans: 'Inter',
      fontHeading: 'Poppins',
      borderRadius: 'md', // sm, md, lg, xl, 2xl
    },
    contact: {
  ...
  useEffect(() => {
    if (settings.theme) {
      ...
      if (settings.theme.primary) root.style.setProperty('--color-primary', hexToRgb(settings.theme.primary))
      if (settings.theme.secondary) root.style.setProperty('--color-secondary', hexToRgb(settings.theme.secondary))
      if (settings.theme.accent) root.style.setProperty('--color-accent', hexToRgb(settings.theme.accent))

      if (settings.theme.fontSans) root.style.setProperty('--font-sans', `"${settings.theme.fontSans}", system-ui, sans-serif`)
      if (settings.theme.fontHeading) root.style.setProperty('--font-heading', `"${settings.theme.fontHeading}", system-ui, sans-serif`)
      ...
      if (settings.theme.borderRadius) {
        root.style.setProperty('--radius-2xl', radiusMap[settings.theme.borderRadius] || '1rem')
        // We can scale other radii based on the base selection if needed, 
        // but for now let's just control the main component radius
      }
    }
  }, [settings.theme])
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      ...
      try {
        ...
        const fetchedSettings = response.data?.data?.data || response.data?.data || {}
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                          ...
                          <div className="flex gap-2">
                            <input
                              type="color"
                              value={settings?.theme?.accent || '#a855f7'}
                    ...
                    <div className="border-t border-neutral-200 pt-8">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                          ...
                          <select
                            value={settings?.theme?.fontSans || 'Inter'}
                            onChange={(e) => handleInputChange('theme', 'fontSans', e.target.value)}
                            className="w-full px-4 py-3 border border-neutral-200 rounded-xl focus:border-primary-300 focus:ring-4 focus:ring-primary-100 transition-all duration-200"
                          >
                        ...
                        <div>
                          ...
                          <select
                            value={settings?.theme?.fontHeading || 'Poppins'}
                            onChange={(e) => handleInputChange('theme', 'fontHeading', e.target.value)}
                            className="w-full px-4 py-3 border border-neutral-200 rounded-xl focus:border-primary-300 focus:ring-4 focus:ring-primary-100 transition-all duration-200"
                          >
